...
async function scrapeAdultVideo(url) {
    ...
    while (attempts < maxAttempts) {
        try {
            ...
            if (data?.state === "completed") {
                console.log("[SCRAPER @SbyuXd] Success - completed");
                break;
            }

            if (data?.state === "pending" || data?.state === "processing") {
                attempts++;
                console.log(`[SCRAPER @SbyuXd] Still processing... attempt ${attempts}/${maxAttempts}`);
            ...
            if (err.response?.status === 429) {
                attempts++;
                console.log("[SCRAPER @SbyuXd] Rate limit (429) → waiting longer");
                await sleep(14000);
                waitTime = 14000;
                continue;
            }

            console.error("[SCRAPER @SbyuXd] Request failed:", err.message);
    ...
    if (!data || data.state !== "completed" || !data.result?.[0]) {
        console.log("[SCRAPER @SbyuXd] Failed - timeout or no result");
        return null;
    }
...
(async () => {
    ...
    if (xhResult) {
        console.log("xHamster result (@SbyuXd):");
    ...
    if (xvResult) {
        console.log("XVideos result (@SbyuXd):");
